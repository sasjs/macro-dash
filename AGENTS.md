...
- Services return the standard automatic fields (`_PROGRAM`, `SYSDATE`, `SYSTIME`, `_METAUSER`, `SYSCC`) plus data tables.
- Every call has a hard 5s timeout; failure degrades to localStorage mode.
- **The `<sasjs configured="...">` attribute is the config flag**: the `configure` service rewrites the streamed `index.html` itself — on SASjs Server via Drive API `GET`/`PATCH` (`%ms_getfile`/`%mx_createfile`), on Viya via a `filesrvc` fileref on the Drive-hosted `MacroDash.html` (same stamp pass that writes `contextname`/`runAsTask`/`useComputeApi`) — so the page knows synchronously at load. When unconfigured the frontend calls NO services except `configure` (there is no getconfig service — the stamp IS the answer; when configured, only `getscores`/`savescore` are used). Settings persist as a plain `settings.sas` file directly under the apploc (written by `%mx_createfile`, read back by `md_init` — `filesrvc` fileref on Viya, `%ms_getfile` on SASjs Server); there is no settings job and no `jobs/` folder.
...
- JS stored-program runtime gotchas (see the `sasjs-server` skill for the full list):
  ...
  - mocks persist state WITHOUT /tmp: settings on the Drive (`<drive>/macrodash.settings.json`, derived from `weboutPath` → `../../../drive`), scores as `scores.json` inside the configured rootdir (a real local folder - the analogue of `md_rootdir/scores.sas7bdat`)
