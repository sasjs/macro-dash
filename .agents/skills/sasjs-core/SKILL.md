...
- Tests live in `tests/<platform>/` (e.g. `tests/x-platform/mx_foo.test.sas`) and are run with `sasjs test`.
- **No open (non-macro) code that depends on runtime conditions**: wrap platform-branching or stateful blocks in `%macro ... %mend` and call them — open `%if`/`%else` and macro-variable mutations at the top level of a test program do not behave as expected when the test harness compiles/runs tests.
...
- `mx_createfile` (cross-platform file write) now exists in `xplatform/`: `%ms_createfile` on SASjs, `%mv_createfile` on Viya, and on SAS 9 (which has no file concept) `%mm_createstp(stptype=2)` + `%mm_updatestpsourcecode` to store the content in metadata. Viya has no content-file READER (`mv_getjobcode` reads jobs only) — tests can only assert existence there (`mfv_existfile`).
- Check `node_modules/@sasjs/core/<platform>/` for the actual macro inventory before referencing one — the compile step (`sasjs compile`) fails with "Unable to locate dependencies" for non-existent macros listed in `<h4> SAS Macros </h4>` headers.
